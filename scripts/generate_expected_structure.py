# scripts/generate_expected_structure.py
import json
import re
import argparse

# ---------- Arguments ----------
parser = argparse.ArgumentParser(description="Generate expected structure JSON from CFR Markdown")
...
#            parent = f"{current_req}{suffix[0]}"
        ...
        # Always ignore suffix length
        parent = current_req

        if parent in requirements:
            requirements[parent].append(suffix)
        else:
            requirements[parent] = [suffix]
